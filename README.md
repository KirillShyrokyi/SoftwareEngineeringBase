...
| Раздел | Что внутри |
|---|---|
| **[ROADMAP.md](ROADMAP.md)** | Полная карта навыков Junior C#/.NET и порядок изучения |
| **[STUDY_SYSTEM.md](STUDY_SYSTEM.md)** | Как мы разбираем каждую тему, чтобы знания связывались между собой |
...
В дорожной карте используются четыре состояния:
...
- почему зависимость имеет конкретный lifetime;
- почему DTO отличается от domain/database entity;
- почему запрос выполняется асинхронно;
- где должна жить бизнес-логика;
- зачем нужна transaction;
- как API превращает ошибку в HTTP response;
- как данные проходят путь от HTTP request до database.
